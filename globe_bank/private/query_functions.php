<?php
    ...
    function validate_page($page) {
        ...
        // page name unique?
//         if(!has_unique_page_name($page)) {
//             $errors[] = "Page Name exists for that Subject.";
//         }
        
        return $errors;
    }
        
    function has_unique_page_name($page) {
        global $db;
        
        $sql = "SELECT subject_id, menu_name FROM pages ";
        $sql .= "WHERE subject_id='" . db_escape($db, $page['subject_id']) ."' AND ";
    ...
    function validate_admin($admin, $options=[]) {
        ...
        if(is_blank($admin['username'])) {
            $errors[] = "Userame cannot be blank.";
        ...
        if ($password_required) {
            ...
            $confirm_password = (string) $admin['confirm_password'];
            if(is_blank($password)) {
                $errors[] = "Password cannot be blank.";
            } elseif(!has_length($admin['password'], ["min => 12"])){
                $errors[] = "Password must be at least 12 characters.";
            }elseif($password !== $confirm_password) {
                $errors[] = "Password does not equal Confirm Password";
            }elseif(has_number($admin['password']) === false) {
                $errors[] = "Password must include at least one number.";
            }elseif(has_lower_case($admin['password']) === false) {
                $errors[] = "Password must include at least one lower case letter.";
            }elseif(has_upper_case($admin['password']) === false) {
                $errors[] = "Password must include at least one upper case letter.";
            }elseif(has_symbol($admin['password']) === false) {
                $errors[] = "Password must include one symbol: $ ~ ! ^ [  ] & %.";
            }
        }
    ...
    function has_unique_admin_name($admin) {
        global $db;
        
        $sql = "SELECT username FROM admins ";
        $sql .= "WHERE username='" . db_escape($db, $admin['username']) ."'";
        $results = mysqli_query($db, $sql);
        $row_count = mysqli_num_rows($results);
        mysqli_free_result($results);
    ...
    function find_subject_by_id($id, $options = []){
        ...
        $visible = $options['visible'] ?? 'false';
        $sql = "SELECT * FROM subjects ";
        $sql .= "WHERE id='" . db_escape($db, $id) ."' ";  //use single quotes to guard against sql injection!
        if($visible) {
    ...
    function find_subject_name_by_id($id) {
        ...
        $results = mysqli_query($db, $sql);
        confirm_result_set($results);
        $subject = mysqli_fetch_assoc($results);
        mysqli_free_result($results);
        return $subject;
    }
    ...
    function find_pages_by_subject_id($subject_id, $options = []){
        ...
        $visible = $options['visible'] ?? 'false';
        $sql = "SELECT * FROM pages ";
        $sql .= "WHERE subject_id='" . db_escape($db, $subject_id) ."' ";  //use single quotes to guard against sql injection!
        if ($visible) {
            $sql .= "AND visible = true ";
        }
        $sql .= "ORDER BY position ASC";
        ...
        return $result; //returns assoc array
    }
    ...
    function insert_admin($admin) {
        ...
        if(!has_unique_admin_name($admin)) {
    ...
    function find_admin_by_id($id){
        ...
        $sql = "SELECT id, first_name, last_name, email, username FROM admins ";
        $sql .= "WHERE id='" . db_escape($db, $id) ."' ";  //use single quotes to guard against sql injection!
        $sql .= "LIMIT 1";
        $result = mysqli_query($db, $sql);
        confirm_result_set($result);
    ...
    function find_admin_by_username($username){
        ...
        $sql = "SELECT * FROM admins ";
        $sql .= "WHERE username='" . db_escape($db, $username) ."' ";  //use single quotes to guard against sql injection!
        $sql .= "LIMIT 1";
        $result = mysqli_query($db, $sql);
        confirm_result_set($result);
